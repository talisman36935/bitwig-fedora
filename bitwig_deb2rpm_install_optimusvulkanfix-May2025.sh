...
# Create wrapper script for NVIDIA Vulkan fix
echo "Creating NVIDIA Vulkan wrapper script..."
cat > /usr/local/bin/bitwig-studio << 'EOF'
#!/bin/bash
cd /opt/bitwig-studio
exec env __NV_PRIME_RENDER_OFFLOAD=1 __GLX_VENDOR_LIBRARY_NAME=nvidia VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/nvidia_icd.json ./bitwig-studio "$@"
EOF

chmod +x /usr/local/bin/bitwig-studio
echo "Created NVIDIA Vulkan wrapper: /usr/local/bin/bitwig-studio"
...
# Fix paths in desktop files to use our wrapper script
for desktop_file in /usr/local/share/applications/*.desktop; do
if [ -f "$desktop_file" ]; then
# Update Exec path to use /usr/local/bin/bitwig-studio (our wrapper)
sed -i 's|Exec=.*bitwig-studio.*|Exec=/usr/local/bin/bitwig-studio %U|g' "$desktop_file"
echo "Updated paths in $desktop_file to use NVIDIA Vulkan wrapper"
fi
done
fi
...
echo "Executable wrapper: /usr/local/bin/bitwig-studio"
echo ""
echo "The installation includes NVIDIA Vulkan optimization for"
echo "hybrid graphics systems. The desktop shortcut will"
echo "automatically use the correct GPU settings."
echo ""
...
echo
echo "Installation complete! Your dock shortcut will now use NVIDIA Vulkan automatically."